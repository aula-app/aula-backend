...
class Idea
{
  ...
  public function getWildIdeasByUser($user_id, $offset = 0, $limit = 0, $orderby = 4, $asc = 0, $status = -1, $extra_where = "")
  {
    ...
    */
    ...
    $select_part = 'SELECT ' . $this->db->au_users_basedata . '.displayname,' . $this->db->au_users_basedata . '.id as user_id, ' . $this->db->au_ideas . '.room_id, ' . $this->db->au_ideas . '.created, ' . $this->db->au_ideas . '.last_update, ' . $this->db->au_ideas . '.id, ' . $this->db->au_ideas . '.title, ' . $this->db->au_ideas . '.content, ' . $this->db->au_ideas . '.sum_likes, ' . $this->db->au_ideas . '.sum_comments, ' . $this->db->au_ideas . '.sum_votes , ' . $this->db->au_ideas . '.number_of_votes FROM ' . $this->db->au_ideas;
