...
class User {
    ...
    */
    ...
    public function getMissingConsents ($user_id) {
      ...
      $stmt = $this->db->query('SELECT '.$this->db->au_texts.'.id, '.$this->db->au_texts.'.headline, '.$this->db->au_texts.'.body, '.$this->db->au_texts.'.consent_text, '.$this->db->au_consent.'.consent FROM '.$this->db->au_texts.' WHERE '.$this->db->au_texts.'.id NOT IN ('.implode(",", $ids).') AND '.$this->db->au_texts.'.user_needs_to_consent = 2');
